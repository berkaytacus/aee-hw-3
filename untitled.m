...
n_max_prop = sqrt(p.*V.^2./(2*K*W_0/S).*(P_A_max./V./W_0 - (p.*V.^2.*CD0 * S./(2*W_0))));
...
%% Q5
clc
Vm = (2 * P_A_max / (S*(K*CLmax^2 + CD0)))^(1/3)
n = Vm^2*S*CLmax/W_0/2

R1 = 30^2 ./ (9.8 * sqrt(n1^2-1))
Rm = Vm^2 ./ (9.8 * sqrt(n^2-1))
